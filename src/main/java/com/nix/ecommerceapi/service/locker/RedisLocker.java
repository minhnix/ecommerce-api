...
public class RedisLocker implements DistributedLocker {
    private static final int DEFAULT_RETRY_TIME_MS = 10;
    private final RedisTemplate<String, String> redisTemplate;

    @Override
    public <T> LockExecutionResult<T> lock(String key, int maxRetryNumber, long lockTimeOut, Callable<T> task) {
        try {
            return tryToGetLock(() -> {
    ...
    private static <T> T tryToGetLock(final Supplier<T> task, final String key, final int maxRetryNumber) throws Exception {
